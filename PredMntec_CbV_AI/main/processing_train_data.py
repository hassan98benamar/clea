...
from PredMntec_CbV_AI.config_mongo import get_database
...
class ProcessTrain():
    ...
    def read_data(self, ctrl_name=None, usecase = None):
        print('read data mongodb')
        db = get_database()
        control_collection = db['captures']
        ...
        for doc in cursor:
            _id = doc.get('_id')
            attribute = ""
            ai_status = doc['status']['ai']
            user_status = doc['status']['user']
            timestamp = doc['creation']
            dateWithoutTimeStamp = None
            if doc['creation'] != '':
                dateWithoutTimeStamp = datetime.strptime(doc['creation'], '%Y-%m-%dT%H:%M:%S.%fZ').date()

            if doc['result']!=[] and doc['result']['subjects'] != []:
                subjectSize = len(doc['result']['subjects'])
                for k in range(subjectSize):
                    if doc['result']['subjects'][k]['detections'] != []:
                        detectionSize = len(doc['result']['subjects'][k]['detections'])
                        for l in range(detectionSize):
                            dict_tmp = {}
                            dict_global_tmp = {}
                            ctrlName = doc['result']['subjects'][k]['detections'][l]['reference']
                            #dict_tmp["#"] = i
                            dict_tmp["_id"] = _id
                            dict_tmp["Attribute"] = attribute
                            dict_tmp["Control"] = ctrlName
                            dict_tmp["Ai Status"] = ai_status
                            dict_tmp["User Status"] = user_status
                            dict_tmp["Date"] = dateWithoutTimeStamp.strftime('%Y-%m-%d')
                            dict_tmp["Timestamp"] = timestamp

                            dict_global_tmp["_id"] = _id
                            dict_global_tmp["Date"] = dateWithoutTimeStamp.strftime('%Y-%m-%d')
                            dict_global_tmp["Timestamp"] = timestamp
                            dict_global_tmp["GlobalStatus"] = ai_status

                            data_dict.update( { str(i) : dict_tmp } )
                            data_dict_global.update( { str(i) : dict_global_tmp } )
                            i = i+1

        df = pd.DataFrame(list(data_dict.values()))
        df_global = pd.DataFrame(list(data_dict_global.values()))

        trainedUsecase = None
        if usecase is not None:
            trainedUsecase = usecase
        else:
            trainedUsecase = self.pre.usecase

        # saving df_conformity.pkl file
        path = os.path.join(self.FILES_PATH, 'df_'+trainedUsecase+'.pkl')
        with open(path, 'wb') as f:
            pickle.dump(df, f)
        # saving df_conformity_global.pkl file
        path_global = os.path.join(self.FILES_PATH,  'df_'+trainedUsecase+'_global.pkl')
        with open(path_global, 'wb') as f:
        ...
        if self.pre.useDB:
            ...
            path = os.path.join(self.pre.FILES_PATH, 'df_'+trainedUsecase+'.pkl')
