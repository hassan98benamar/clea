...
class Train:
    ...
    # def training(self, control_name, req_df):
    #     try:
    #         print(control_name)
    ...
    #         data_df = self.process_data(result_df_proc)
    #         # print(data_df)
    #         data_df = self.traindata.feature_eng(data_df)
    #
    #         x_train, y_train, x_test, y_test = self.traindata.train_test_split(data_df)
    #         model, acc, recall, rms = self.traindata.build_model(x_train, y_train, x_test, y_test)
    #
    #         # #
    #         # Save model with version
    #         results = pd.DataFrame(columns=['Control Name', 'Freq', 'Model', 'Recall', 'Accuracy','RMSE'])
    #         results.loc[0] = [control_name, int(ctrl_freq), model, recall, acc, rms]
    #         # print(results)
    #         # exit(0)
    #         path, version = self.save.save_model_by_control_name(results, control_name+'_daily')
    #         print(path)
    #     except Exception as e:
    #         results = {'message': "Model not trained for control: {}".format(control_name)}
    #         version = 'NA'
    #     return control_name, version

    def training(self, control_name, req_df, max_date):
        try:
            print(control_name)
            if not self.pre.useDB:
                ctrl_df = req_df[req_df['Control'] == control_name]
                ctrl_df = ctrl_df.drop(['Control', 'Attribute'], axis=1)
            else:
                ctrl_df = req_df.copy()
    ...
    def train_all_controls(self):
        ...
        # response_result = [response_result[i][1:] for i in range(len(response_result))]
        print(response_result)
        # exit(0)

        # for control_name in set(req_df['Control']):
        #     print(control_name)
        #     results, version = self.training(control_name, req_df)
